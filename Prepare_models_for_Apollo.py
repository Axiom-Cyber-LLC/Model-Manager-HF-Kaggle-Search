...
def main() -> int:
    ap = build_parser("Prepare local GGUF models for Apollo by symlinking them into Apollo's model directory.", HOME / 'Library' / 'Application Support' / 'Apollo' / 'models')
    args = ap.parse_args()
    return run_linker('Apollo', args.target, args, nested=False)
