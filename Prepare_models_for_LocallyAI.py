...
def main() -> int:
    ap = build_parser("Prepare local GGUF models for LocallyAI by symlinking them into LocallyAI's model directory.", HOME / 'Library' / 'Application Support' / 'LocallyAI' / 'models')
    args = ap.parse_args()
    return run_linker('LocallyAI', args.target, args, nested=False)
